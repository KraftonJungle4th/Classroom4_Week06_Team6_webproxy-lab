...
int parse_uri(char *uri, char *filename, char *cgiargs)
{
    ...
    { /* Static content */                  // line:netp:parseuri:isstatic
        ...
        // else if (strcmp(uri + strlen(uri) - 4, "/adder") == 0)
...
{
    int src_fd;
    char *src_pointer, filetype[MAXLINE], buf[MAXBUF];
...
Connection: close\r\nContent-length: %d\r\nContent-type: %s\r\n\r\n",
    ...
    src_pointer = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, src_fd, 0);
    Close(src_fd);
    Rio_writen(fd, src_pointer, filesize);
    Munmap(src_pointer, filesize);
}
